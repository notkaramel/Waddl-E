...
# Import Subsystems
from subsystem.Vehicle import goStraight, turnAround, slightTurn, stop
from subsystem.Vehicle import getFrontColor, betterTurn
from subsystem.Delivery import getSideColor, deliverCube, resetRack
from subsystem.components.Button import READY_BUTTON, STOP_BUTTON
...
Each action will be triggered based on the condition changes
...
def WaddlEGoesNormally():
    ...
    while True:
        ...
        if REMAINING_CUBES == 0:
            print(
                "YAY! Waddl-E has finished her job.\
...
def WaddlETriesToCatchColor() -> str:
    ...
    while True:
        ...
        # elif sideColor == 'white': # ahead of the zone | ? before the zone
            ...
            # # go backward to catch the color again
        else:
            toBeDelivered = sideColor
            if DEBUG:
...
def WaddleMain():
    ...
    WaddleReset()
    print(
        "Please reload the cubes in order: [RED, ORANGE, YELLOW, \
            GREEN, BLUE, PURPLE]\nPress the button when done.")
    while True:
        if READY_BUTTON.is_pressed():
            break
    print('Departure in 1 second!')
    sleep(1)
